...
occ <- read.csv("Data/Promops centralis.csv", header = T)

#########################################
#Load M polygon

Mpol <- readOGR("Data/shp/Promops centralis_mcp.shp") %>% gBuffer(width = 2.5)
...
bg.xy <- read.csv("Data/BackGround_P_Centralis.csv")[,c(2,3)]
#write.csv(bg.xy, "Data/BackGround_P_Centralis.csv")
...
t <- thin(occ, long.col = "Lon", lat.col = "Lat", spec.col = "Sp", thin.par = 10, reps = 100, locs.thinned.list.return = T, write.files = F)
t[[1]]
...
#write.csv(evalTbl, file = "Maxent/P_centralis/Table_Models.csv", quote = F, row.names = F)
#save(e, file = "Maxent/P_centralis/e.rda")
...
AUC
 ...
 #Se guardan los outputs de MaxEnt para cada modelo (Comando solo para Linux, Windows a manita)
system(paste0("cp -R ",evalMods$rm.5_fc.LQHP@path," ", getwd(),"/Maxent/P_centralis/","LQHP_5")) 
#AUC

#Guardar Info Importante de Cada Modelo

png("Maxent/P_centralis/LQHP_5/Var_Response.png"); dismo::response(evalMods$rm.5_fc.LQHP); dev.off()

evalMods$LQ_6@lambdas
evalMods$LQH_6@lambdas
evalMods$H_2@lambdas
...
predM <- dismo::predict(evalMods$rm.6_fc.LQHP, envsBgMsk, args="outputformat=cloglog")
...
#PartialROC Resultas
#LQHP4 =    Mean_AUC     Mean_pAUC_ratio_at_5%           P_value 
#          0.5992578          1.1321984                0.0000000 

#LQHP_6 =  Mean_AUC     Mean_pAUC_ratio_at_5%           P_value 
#         0.6443271           1.1900429                 0.000000 
...
predG <- dismo::predict(evalMods$rm.6_fc.LQHP, predsProj, args="outputformat=cloglog")
plot(predG)
...
writeRaster(predM, filename = "Maxent/P_centralis/LQHP_5/predM_LQHP5", format = "GTiff")
writeRaster(predG, filename = "Maxent/P_centralis/LQHP_5/predG_LQHP5", format = "GTiff")
writeRaster(pred.binM, filename = "Maxent/P_centralis/2023/LQHP_6//pred_binM_TES2", format = "GTiff")
writeRaster(pred.binG, filename = "Maxent/P_centralis/LQHP_5/pred_binG_LQHP5", format = "GTiff")
